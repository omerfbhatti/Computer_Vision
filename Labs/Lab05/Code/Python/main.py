...
class Tracker:
    ...
    def get_inliers_from_RANSAC(self, inlier_mask, good, matched1, matched2, matched1_keypoints, matched2_keypoints):
        inliers1 = []
        inliers2 = []
        inliers1_keypoints = []
        inliers2_keypoints = []
    ...
    def drawLine(self, lines1, lines2, img):
        ...
        #    img = cv2.line(img, (x0,y0), (x1,y1), (0, 0, 255), 1)
        ya = 0
    ...
    def process(self, frame):
        ...
        inlier_matches, inliers1, inliers2, inliers1_keypoints, inliers2_keypoints = self.get_inliers_from_RANSAC(inlier_mask, good,                                        
        ...
        print("inliers1: ", inliers1.shape)
        print("inliers2: ", inliers2.shape)
        P_options = getReconstructedP_Matrix(E)
        P = getCorrectP_prime(P_options, inliers1, inliers2)
        #print("P: ", P)
        R = P[:,:3]
        T = P[:,3:]
        print("R_from Essential Matrix: ", R)
        print("T_from Essential Matrix: ", T)
        ...
        eplines1 = cv2.computeCorrespondEpilines(inliers2.reshape(-1,1,2), 2, E)
        eplines1 = eplines1.reshape(-1, 3)
        
        eplines2 = cv2.computeCorrespondEpilines(inliers1.reshape(-1,1,2), 1, E)
        eplines2 = eplines2.reshape(-1,3)
        
        first_frame = self.drawLine(eplines1, eplines2, self.first_frame.copy())
...
def main():
    ...
    width  = video_in.get(cv2.CAP_PROP_FRAME_WIDTH)   # float `width`
    height = video_in.get(cv2.CAP_PROP_FRAME_HEIGHT)  # float `height`
    ratio = 640.0 / width
    ...
    cv2.namedWindow(video_name, cv2.WINDOW_NORMAL | cv2.WINDOW_KEEPRATIO | cv2.WINDOW_GUI_EXPANDED);
    ...
    stat_o = orb_tracker.setFirstFrame(frame, bb, "ORB");  # returns no. of keypoints detected
    ...
    while True:
        ...
        res_frame = cv2.resize(res_frame, frame_size)
        vidWriter.write(res_frame)

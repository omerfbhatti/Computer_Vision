import cv2
import numpy as np
from camera_calibration2 import *
...
if __name__ == '__main__':
    ...
    while (key < 0):        # play video until press any key
        ...
        # RECTIFYING RADIAL AND TANGENTIAL DISTORTION
        matFrameDisplay = showUndistorted(matFrameDisplay, mtx, dist)
        matFrameCapture = showUndistorted(matFrameCapture, mtx, dist)
    ...
    if (matFrameCapture is not None):
        ...
        if (len(state.pts) == 4):
            ...
            fs = cv2.FileStorage(homography_file, cv2.FILE_STORAGE_WRITE)
